...
Update the help text show how to install the bash_completion Linux show here
[Kubectl docs show mac options](https://kubernetes.io/docs/tasks/tools/install-kubectl/#enabling-shell-autocompletion)
...
**Note:** The cobra generator may include messages printed to stdout for example
if the config file is loaded, this will break the auto complete script
...
`out.sh` will get you completions of subcommands and flags. Copy it to
`/etc/bash_completion.d/` as described
[here](https://debian-administration.org/article/316/An_introduction_to_bash_completion_part_1)
and reset your terminal to use autocompletion. If you make additional
...
show the list of nouns in this example again instead of the replication
controllers.
...
In this example we use --filename= and expect to get a json or yaml file as the
argument. To make this easier we annotate the --filename flag with valid
filename extensions.
...
cobra.BashCompFilenameExt, you can specify a custom flag completion function
with cobra.BashCompCustom:
...
You can also configure the `bash aliases` for the commands and they will also
support completions.
